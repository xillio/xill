...
//		}).start();
	...
	/**
	 * Pretty prints a timestamp for the provided timeremaining.
	 * e.g. 2 hours, 16 minutes, 1 second
	 *
	 * @param timeremaining
	 *        a number of milliseconds
	 * @return The formatted timestamp
	 */
	private static String formatTimeRemaining(final long timeremaining) {
		if (timeremaining < 0) {
			return "unknown.";
		}
		...
		long minutes = TimeUnit.MILLISECONDS.toMinutes(timeremaining - TimeUnit.HOURS.toMillis(hours));
		if (minutes > 1) {
		...
		long seconds = TimeUnit.MILLISECONDS.toSeconds(timeremaining - TimeUnit.HOURS.toMillis(hours) - TimeUnit.MINUTES.toMillis(minutes));
		if (seconds > 1) {
		...
		// No time remaining. Let's print that!
		if (time.length() == 0) {
			time = "0 seconds";
		}
		return time.trim();
