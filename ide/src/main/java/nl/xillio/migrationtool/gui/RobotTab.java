...
public class RobotTab extends Tab implements Initializable, ChangeListener<DocumentState> {
	...
	private static void initializeSettings(final File documentPath) {
		settings.simple().register("Layout", "RightPanelWidth_" + documentPath.getAbsolutePath(), "0.7", "Width of the right panel for the specified currentRobot");
		settings.simple().register("Layout", "RightPanelCollapsed_" + documentPath.getAbsolutePath(), "true", "The collapsed-state of the right panel for the specified currentRobot");
		settings.simple().register("Layout", "EditorHeight_" + documentPath.getAbsolutePath(), "0.6", "The height of the editor");
	}
	...
	private void initializeTab(final File documentPath) {
		...
		double editorHeight = Double.parseDouble(settings.simple().get("Layout", "EditorHeight_" + documentPath.getAbsolutePath()));
		...
		spnBotLeft.getDividers().get(0).positionProperty().addListener(
			(observable, oldPos, newPos) -> {
				...
				settings.simple().save("Layout", "EditorHeight_" + documentPath.getAbsolutePath(), Double.toString(height));
			});
	...
	public void initialize(final URL arg0, final ResourceBundle arg1) {
		...
		Platform.runLater(() -> {
			...
			boolean showRightPanel = Boolean.parseBoolean(settings.simple().get("Layout", "RightPanelCollapsed_" + getDocument().getAbsolutePath()));
	...
	private void hideButtonPressed() {
		File document = processor.getRobotID().getPath();
		if (document != null) {
			settings.simple().save("Layout", "RightPanelCollapsed_" + document.getAbsolutePath(), "true");
			if (!spnBotPanes.getDividers().isEmpty()) {
				settings.simple().save("Layout", "RightPanelWidth_" + document.getAbsolutePath(), Double.toString(spnBotPanes.getDividerPositions()[0]));
			}
		}
	...
	private void showButtonPressed() {
		File document = processor.getRobotID().getPath();
		settings.simple().save("Layout", "RightPanelCollapsed_" + document.getAbsolutePath(), "false");
		...
		spnBotPanes.setDividerPosition(0, Double.parseDouble(settings.simple().get("Layout", "RightPanelWidth_" + document.getAbsolutePath())));
		spnBotPanes.getDividers().get(0).positionProperty().addListener((position, oldPos, newPos) -> {
			if (spnBotPanes.getItems().contains(vbxDebugpane)) {
				settings.simple().save("Layout", "RightPanelWidth_" + document.getAbsolutePath(), newPos.toString());
			}
		});
	}
