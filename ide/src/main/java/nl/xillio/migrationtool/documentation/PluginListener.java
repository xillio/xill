...
public class PluginListener {
	...
	public void pluginLoaded(final PluginPackage plugin) {
		...
		for (Construct construct : plugin.getConstructs()) {
			...
			try {
				...
				FileUtils.write(new File("./helpfiles/packages/" + plugin.getName() + ".html"), packageDocumentation(plugin.getName(), functions).toHTML());
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	 ...
	 */
	private HtmlGenerator packageDocumentation(final String packageName, final List<FunctionDocument> functions)
	{
