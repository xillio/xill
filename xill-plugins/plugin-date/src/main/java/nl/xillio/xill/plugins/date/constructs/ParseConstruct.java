...
public class ParseConstruct extends BaseDateConstruct {
	...
	return new ConstructProcessor(ParseConstruct::process, new Argument("date", ExpressionBuilder.NULL), new Argument("format", ExpressionBuilder.NULL));
    }
	...
	if (dateVar.getType() != ExpressionDataType.ATOMIC) {
	    throw new RobotRuntimeException("Expected ATOMIC value for date");
	}
	if (dateVar.getType() != ExpressionDataType.ATOMIC) {
	    throw new RobotRuntimeException("Expected ATOMIC value for format");
	}
    ...
    private static ZonedDateTime parseAsDate(final MetaExpression dateVar, final MetaExpression formatVar) throws DateTimeParseException {
