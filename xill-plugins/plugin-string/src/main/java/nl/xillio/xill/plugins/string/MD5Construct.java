...
public class MD5Construct implements Construct {
	...
	private static MetaExpression process(final MetaExpression valueVar) {
		...
		// md5 = MD5(valueVar.getStringValue());

		// return md5.isPresent() ? ExpressionBuilder.fromValue(md5.get()) :
		// ExpressionBuilder.NULL;
	}
	/*
	 * public static Optional<String> MD5(final String value) { Optional<String>
	 * md5 = Optional.empty(); try (ByteArrayInputStream input = new
	 * ByteArrayInputStream(value.getBytes("UTF-8"))) { md5 =
	 * Optional.of(MD5(input));
	 * 
	 * } catch (IOException e) { throw new RobotRuntimeException("IOException");
	 * } return md5; }
	 * 
	 * private static String MD5(final InputStream in){ MD5 digest = new MD5();
	 * digest.Init();
	 * 
	 * // This bit causes memory issues with big files // byte[] buff =
	 * IOUtils.readFully(in, -1, false);
	 * 
	 * int n; byte[] buff = new byte[8192]; while ((n = in.read(buff)) > 0) {
	 * digest.Update(buff, n); } digest.Final(); return digest.asHex(); }
	 */
}