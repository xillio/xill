...
public class HungarianAlgorithmConstruct implements HelpComponent {
	...
	return new ConstructProcessor(HungarianAlgorithmConstruct::process, new Argument("matrix"),
		new Argument("max"));
	...
	if (matrixVar.getType() == ExpressionDataType.LIST) {
		...
		for (int j = 0; j < columns; j++) {
			...
			throw new RobotRuntimeException("Invalid value `"
				+ MetaExpression.extractValue(((List<MetaExpression>) matrix.get(j).getValue()).get(j))
				+ "` in matrix at [" + i + "," + j + "]");
		    }
		}
	    }
		...
		item.put("row", ExpressionBuilder.fromValue(element[0]));
		item.put("col", ExpressionBuilder.fromValue(element[1]));
    ...
    private static int hg_step2(int step, final double[][] cost, final int[][] mask, final int[] rowCover,
	    final int[] colCover) {
	// What STEP 2 does:
    ...
    private static int hg_step4(int step, final double[][] cost, final int[][] mask, final int[] rowCover,
	    final int[] colCover, final int[] zero_RC) {
	// What STEP 4 does:
    ...
    // Aux 1 for hg_step4.
    private static int[] findUncoveredZero(final int[] row_col, final double[][] cost, final int[] rowCover,
	    final int[] colCover) {
    ...
    private static int hg_step5(int step, final int[][] mask, final int[] rowCover, final int[] colCover,
	    final int[] zero_RC) {
	// What STEP 5 does:
    ...
    private static int hg_step6(int step, final double[][] cost, final int[] rowCover, final int[] colCover,
    ...
    // Aux 1 for hg_step6.
    private static double findSmallest(final double[][] cost, final int[] rowCover, final int[] colCover,
	    final double maxCost) {
