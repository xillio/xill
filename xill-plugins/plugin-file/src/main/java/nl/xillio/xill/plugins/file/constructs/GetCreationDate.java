 ...
 */
public class GetCreationDate extends Construct {
     ...
     */
    @Inject
    public GetCreationDate(final DateFactory d, final FileUtilities f) {
        this.date = d;
        this.fileUtils = f;
    }
    ...
    static MetaExpression process(final ConstructContext c, final DateFactory date, final FileUtilities io, final MetaExpression expr) {
        try {
            Date d = date.from(io.getCreationDate(getFile(c, expr.getStringValue())).toInstant());
